...
class NotificationSettings(Base):
    ...
    last_whatsapp_gateway_status_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    last_whatsapp_disconnect_alert_at: Mapped[Optional[datetime]] = mapped_column(DateTime(timezone=True), nullable=True)
    updated_by: Mapped[Optional[str]] = mapped_column(String(120), nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, nullable=False)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utcnow, onupdate=utcnow, nullable=False)
...
class NotificationRecipient(Base):
    ...
    whatsapp_numbers_json: Mapped[list[str]] = mapped_column(JSON, default=list, nullable=False)
